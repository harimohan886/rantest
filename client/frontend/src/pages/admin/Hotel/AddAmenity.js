...
export default function AddAmenity() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${process.env.REACT_APP_BASE_URL}/hotel/amenities/`, data);
    ...
    <div className="relative md:ml-64 bg-default-skin">
      ...
      <div className="flex flex-wrap min600">
        <div className="w-full mb-12 xl:mb-0 px-4 padding-top80">
          <div className='mt-4'>
            <h1 className='text-2xl text-black font-bold mb-3'>Add Amenity</h1>
          </div>
          <form onSubmit={handleSubmit}>
            <div className="mb-6">
              <label className="block mb-2 text-sm font-bold text-gray-900 dark:text-gray-300">Amenity</label>
              <input type="text" id="amenity" onChange={(e) => setAmenity(e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required="" />
            </div>
            <div className='mb-6'>
