...
export default function PackagePricing({ optionData, packageName, setData }) {
    ...
    const RoomInc = () => {
        ...
        console.log(safariCost);
        console.log(opData.price);
        console.log(ead);
        console.log(ecc);
       ...
       if(date != null){
         axios.post(`${process.env.REACT_APP_BASE_URL}/package/packages/getOptions`,{date:date}).then((res)=>{
            ...
            if(res.data.block == 1){ swal('Booking Not Available',''); } 
               ...
               setOpData(pricedata);
               document.querySelector('.final-price').innerHTML = pricedata.price + pricedata.safari_price;
                            ...
                            {optionData?.map((option, key) => {
                                ...
                                return (
                                    <tr key={key.toString()} className="package-group">
                                        ...
                                        <td className='text-center'>
                                            <input type="date" onChange={(e) => CheckOptions(e, e.target.value)} className="input-travel-date travel-date form-control" name="travel_date" min={moment().format("YYYY-MM-DD")} />
                                        </td>
                                        <td className='text-center'>
